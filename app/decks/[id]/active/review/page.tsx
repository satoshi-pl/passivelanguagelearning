...
import ActiveReviewPageClient from "./ActiveReviewPageClient";
import { getActiveReviewPageData } from "@/lib/active-review/pageData";
...
function normalizeMode(raw: unknown): ActiveReviewMode {
...
export default async function DeckActiveReviewPage({
  ...
  const warmEntry = getSingleParam(sp.entry).toLowerCase() === "active_dashboard";

  if (warmEntry) {
    return (
      <ActiveReviewPageClient
        deckId={deckId}
  ...
  const result = await getActiveReviewPageData({
    supabase,
    userId: user.id,
    deckId,
    mode,
  ...
  if (result.kind === "error") {
    return (
      <div className="pll-workspace" style={{ maxWidth: 1000, margin: "24px auto", padding: "0 16px" }}>
        <ResponsiveNavLink className="pll-back-link" href={`/decks/${deckId}/active?mode=${mode}`} style={{ textDecoration: "none", color: "inherit" }}>
